...
#weights =  list(class_weight.compute_class_weight(class_weight='balanced', classes=np.unique(y.toxic), y=y.toxic))
#weights.sort()
...
model.fit(train_sentences, train_labels, epochs=100, batch_size=64)
...
preds = model.predict(dataT.comment_text)
preds[preds>=0.5] = 1
preds[preds<0.5] = 0
