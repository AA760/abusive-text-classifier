...
from keras.layers import LSTM, Dense, Dropout, TextVectorization, Bidirectional, Input, Reshape
...
vectorize_layer = TextVectorization(max_tokens=15000, output_mode="tf-idf", ngrams=(2))
...
model.add(tf.keras.layers.Reshape((1, 15000)))
...
model.fit(X_train, y_train, epochs=1, batch_size=64, class_weight=weights, shuffle=True)
model.save('model/')
...
ax.set_title('Confusion Matrix - Bidirectional LSTM w/ tfidf',weight='bold');
