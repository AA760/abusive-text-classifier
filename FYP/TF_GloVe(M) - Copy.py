...
#vectorize_layer = TextVectorization(standardize='lower_and_strip_punctuation', max_tokens=20000, output_mode='int', output_sequence_length=100)
...
pred_padded = pad_sequences(pred_sequences, maxlen=input_length, padding="post", truncating="post")
...
model.fit(train_padded, train_labels, epochs=100, batch_size=64)

model.save('nn_model_embeddings/')

model = load_model('nn_model_embeddings/')

##testing the model
preds = model.predict(pred_padded)
preds[preds>=0.5] = 1
preds[preds<0.5] = 0

preds = np.array(preds)

accuracy = accuracy_score(np.array(yT), preds)
print('Accuracy: %f' % accuracy)

precision = precision_score(np.array(yT), preds, average='weighted')
print('Precision: %f' % precision)

recall = recall_score(np.array(yT), preds, average='weighted')
print('Recall: %f' % recall)

f1 = f1_score(np.array(yT), preds, average='weighted')
print('F1 score: %f' % f1)
